...
void load_env_if_exists(std::filesystem::path const& path) {
  if (std::filesystem::exists(path)) {
    dotenv::init(path.c_str());
  }
}

std::optional<std::string> get_env(char const* name) {
...
void load_env_files() {
  ...
  if (home) {
    load_env_if_exists(std::filesystem::path{home} /
                       ".config/aipp101_chat/.env");
  }
...
Result<Config> resolve_config(CommandLineArgs const& args) {
  Config config{.api_key = ApiKey{"placeholder"},
  ...
  if (args.temperature) {
    ...
    if (end != env->c_str() + env->size()) {
      return make_error("Invalid TEMPERATURE value: '{}'", *env);
    }
