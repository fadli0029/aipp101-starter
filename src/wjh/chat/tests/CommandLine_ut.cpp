...
TEST_SUITE("CommandLine")
{
    ...
    {
        char const * args[] = {"chat_app", "--temperature", "0.7"};
        auto result = parse_args(args);

        REQUIRE(result.has_value());
        REQUIRE(result->temperature.has_value());
        CHECK(static_cast<double>(json_value(*result->temperature)) == doctest::Approx(0.7));
    }

    TEST_CASE("Temperature flag: zero is valid")
    {
        char const * args[] = {"chat_app", "--temperature", "0"};
        auto result = parse_args(args);

        REQUIRE(result.has_value());
        REQUIRE(result->temperature.has_value());
        CHECK(static_cast<double>(json_value(*result->temperature)) == doctest::Approx(0.0));
    }
    ...
    {
        char const * args[] = {"chat_app", "--temperature", "hot"};
        auto result = parse_args(args);

        CHECK_FALSE(result.has_value());
    }

    TEST_CASE("Temperature combined with other flags")
    {
        char const * args[] =
            {"chat_app", "-m", "openai/gpt-4", "--temperature", "0.5", "-t", "1024"};
        auto result = parse_args(args);
